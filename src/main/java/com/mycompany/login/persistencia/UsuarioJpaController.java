...
public class UsuarioJpaController implements Serializable {
    ...
    public void create(Usuario usuario) {
        ...
        try {
            ...
            em.persist(usuario);
            if (unRol != null) {
                unRol.getListaUsusarios().add(usuario);
    ...
    public void edit(Usuario usuario) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            if (unRolOld != null && !unRolOld.equals(unRolNew)) {
                unRolOld.getListaUsusarios().remove(usuario);
                unRolOld = em.merge(unRolOld);
            }
            if (unRolNew != null && !unRolNew.equals(unRolOld)) {
                unRolNew.getListaUsusarios().add(usuario);
    ...
    public void destroy(int id) throws NonexistentEntityException {
        ...
        try {
            ...
            Rol unRol = usuario.getUnRol();
            if (unRol != null) {
                unRol.getListaUsusarios().remove(usuario);
                unRol = em.merge(unRol);
            }
            em.remove(usuario);
            em.getTransaction().commit();
